...
public class ConversationServiceImpl extends ServiceImpl<ConversationMapper, Conversation> implements ConversationService {
    ...
    @Override
    public Long saveMessage(Long conversationId, String role, String content, Integer step, String type) {
        return saveMessage(conversationId, role, content, step, type, null, null);
    }
    ...
    @Override
    public List<Message> getInContextMessages(Long conversationId) {
        // 与 getRecentMessages 相同的过滤条件，但不加 LIMIT：用于上下文压缩策略
        // 判断全量待压缩范围，避免与"仅取最近 N 条"的逻辑重复实现两套过滤条件。
        return messageMapper.selectList(new LambdaQueryWrapper<Message>()
                .eq(Message::getConversationId, conversationId)
                .and(w -> w.in(Message::getType, "user", "summary").or().isNull(Message::getType))
                .orderByAsc(Message::getCreatedAt));
